...
* `python3 ping_scan.py` wait for the results, press Ctl-C
...
## Plots of the latency

```{r}
summary(sample_data$latency)
hist(c(sample_data$latency))
#ggplot(data=sample_data) + aes(x=latency) + geom_histogram(stat="count")
#ggplot(data=c(sample_data$latency)) + geom_histogram(stat="count")
```
